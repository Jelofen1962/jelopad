...
static void notifyServerAddress(void) {
    ...
    for (struct ifaddrs *ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        Notify(TEX_ICON_SYSTEM, "JeloPad Server:\n %s:" REMOTE_PAD_WS_PORT, ip);
    }
    freeifaddrs(ifaddr);
}
...
static void rpc_config_get(struct mg_rpc_req *r) {
    ...
    char buf[1024];
    int pos = 0;
    pos += snprintf(buf + pos, sizeof(buf) - pos, "{\"users\":[");
    for (int i = 0; i < REMOTE_PAD_MAX_USERS; i++) {
        RemoteUser *u = g_remoteUserService->getUserByIndex(i);
        pos += snprintf(buf + pos, sizeof(buf) - pos,
                        "{\"index\":%d,\"enabled\":%s,\"userId\":%d,\"userName\":\"%s\"}%s",
                        u->index, u->enabled ? "true" : "false", u->userId, u->userName,
                        (i < REMOTE_PAD_MAX_USERS - 1) ? "," : "");
    }
    pos += snprintf(buf + pos, sizeof(buf) - pos, "]}");
    mg_rpc_ok(r, "%s", buf);
}
...
static void rpc_config_set(struct mg_rpc_req *r) {
    ...
    if (index >= 0 && index < REMOTE_PAD_MAX_USERS) {
        RemoteUser *u = g_remoteUserService->getUserByIndex(index);
        if (u) {
            u->enabled = enabled;
            u->userId = (int32_t)userId;
            if (name) {
                strncpy(u->userName, name, ORBIS_USER_SERVICE_MAX_USER_NAME_LENGTH);
                u->userName[ORBIS_USER_SERVICE_MAX_USER_NAME_LENGTH] = '\0';
                free(name);
            }

            // Instantly serialize back to /data/GoldHEN/remote_pad.ini
            ini_table_s *table = ini_table_create();
            if (table) {
                const char *section = "default";
                ini_table_read_from_file(table, PLUGIN_CONFIG_PATH);

                char key[32];
                snprintf(key, sizeof(key), "user%ld_enabled", index);
                ini_table_create_entry(table, section, key, u->enabled ? "on" : "off");

                snprintf(key, sizeof(key), "user%ld_id", index);
                char id_str[32];
                snprintf(id_str, sizeof(id_str), "%d", u->userId);
                ini_table_create_entry(table, section, key, id_str);

                snprintf(key, sizeof(key), "user%ld_name", index);
                ini_table_create_entry(table, section, key, u->userName);

                ini_table_write_to_file(table, PLUGIN_CONFIG_PATH);
...
void *websocketThread(void *thread_arg) {
    ...
    mg_rpc_add(&ctx->rpc_head, mg_str("config.get"), rpc_config_get, ctx);
    mg_rpc_add(&ctx->rpc_head, mg_str("config.set"), rpc_config_set, ctx);
...
static int32_t wsInit(RemotePadDriverPtr driver) {
    ...
    int ret = 0;
    if (ctx->running) {
        final_printf("[JeloPad]: ws driver is already initialized\n");
        return 0;
    }
    ...
    if (ret < 0) {
        final_printf("[JeloPad]: Failed to spawn a ws server thread, 0x%X\n", ret);
        return ret;
    }
    return 0;
}
