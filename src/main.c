...
HOOK_DEFINE(scePadInit, void) {
    if (remotePad->initDriver() != SCE_OK) {
        final_printf("Failed to init jelo pad driver\n");
    }
...
int32_t attr_public plugin_load(int32_t argc, const char *argv[]) {
    ...
    g_remoteUserService = initRemoteUserService();
    if (!g_remoteUserService) {
        final_printf("Failed to init jelo user service\n");
        return -1;
    }

    remotePad = initRemotePadService();
    if (!remotePad) {
        final_printf("Failed to init jelo pad service\n");
        return -1;
    }
...
int32_t attr_public plugin_unload(int32_t argc, const char *argv[]) {
    ...
    Notify(TEX_ICON_SYSTEM, "JeloPad Plugin Unloaded");
